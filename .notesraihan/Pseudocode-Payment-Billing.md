...
**Konstanta:** `DISKON_MEMBER = 0.05`  (member dapat potongan 5%)
...
FUNGSI terapkanDiskon() -> angka
    JIKA pelanggan.cekStatusMember() MAKA
        diskon <- subtotal * DISKON_MEMBER      // member: potong 5%
    SELAIN ITU
...
PROSEDUR cetakStruk()
    JIKA pelanggan member MAKA persenDiskon <- 5 SELAIN ITU persenDiskon <- 0
